...
function getAccuracy() {
  ...
  const typedLetters = letters.filter(letter => letter.classList.contains('correct') || letter.classList.contains('incorrect'));
  const correct = typedLetters.filter(letter => letter.classList.contains('correct')).length;
  const total = typedLetters.length;
  return total ? ((correct / total) * 100).toFixed(1) : 0;
}
...
function gameOver() {
  ...
  document.getElementById('info').innerHTML = `WPM: ${Math.round(result)} | Accuracy: ${accuracy}%`;
}
...
document.getElementById('game').addEventListener('keyup', ev => {
  ...
  if (isSpace) {
    if (expected !== ' ') {
      [...document.querySelectorAll('.word.current .letter:not(.correct)')].forEach(letter => addClass(letter, 'incorrect'));
    }
